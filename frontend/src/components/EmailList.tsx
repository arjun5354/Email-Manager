...
export const EmailList = () => {
  ...
  useEffect(() => {
    fetch("https://email-manager-backend-aq68.onrender.com/api/emails") 
      .then((res) => res.json())
      .then((data) => {
        setEmails(data);
        setLoading(false);
  ...
  return (
    ...
    <CardContent className="space-y-4 max-h-[500px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
  {loading ? (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-1 flex-wrap">
              <div>
                ...
                <p className="text-xs text-muted-foreground break-all">{email.senderEmail}</p>
              </div>

              <div className="flex items-center gap-2 shrink-0 text-xs text-muted-foreground w-full sm:w-auto justify-between sm:justify-end mt-1 sm:mt-0">
